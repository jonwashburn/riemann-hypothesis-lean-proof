...
import infrastructure.LegacyNames
...
This file provides lemmas that are missing from mathlib but needed for the proof.
-/

open scoped ENNReal NNReal ComplexConjugate
open Complex WeightedHilbertSpace

namespace MissingLemmas

-- Placeholder for missing lemmas
...
Looking at the lemma to prove and the available infrastructure, I need to show that the arithmetic Hamiltonian H applied to a delta basis function δ_p equals (log p) times δ_p.
...
-- The Hamiltonian H acts on delta basis functions as: H(δ_p) = (log p)δ_p
...
lemma H_deltaBasis_eq : H (deltaBasis q) = (log q) * deltaBasis q := by
  ...
  -- H(δ_p) sends δ_p to (log p)δ_p by definition
  ...
  case pos =>
    ...
    -- Left side: H(δ_q)(q) = (log q) * δ_q(q) = (log q) * 1 = log q

    -- Right side: ((log q) * δ_q)(q) = (log q) * δ_q(q) = (log q) * 1 = log q
  ...
  case neg =>
    ...
    -- Left side: H(δ_q)(p) = (log p) * δ_q(p) = (log p) * 0 = 0

    -- Right side: ((log q) * δ_q)(p) = (log q) * δ_q(p) = (log q) * 0 = 0
...
However, looking more carefully at the infrastructure, I see that the arithmetic Hamiltonian is defined to act as H(ψ)(p) = (log p) * ψ(p) for each prime p. This means we need to be more precise:



lemma H_deltaBasis_eq (q : {p : ℕ // Nat.Prime p}) :

    H (deltaBasis q) = (Real.log q.val : ℂ) • deltaBasis q := by
  ...
  case pos =>
    ...
    -- When p = q: H(δ_q)(q) = (log q) * 1 = log q

    -- and ((log q) • δ_q)(q) = (log q) * 1 = log q
  ...
  case neg =>
    ...
    -- When p ≠ q: H(δ_q)(p) = (log p) * 0 = 0

    -- and ((log q) • δ_q)(p) = (log q) * 0 = 0
...
lemma H_deltaBasis_eq : H (deltaBasis q) = (log q) * deltaBasis q := by
  ...
  -- From the infrastructure: H(δ_p) = (log p)δ_p
...
lemma inner_smul_left : ⟨a • x, y⟩ = a • ⟨x, y⟩
...
lemma inner_smul_left {𝕜 : Type*} {E : Type*} [IsROrC 𝕜] [NormedAddCommGroup E]

    [InnerProductSpace 𝕜 E] (a : 𝕜) (x y : E) : ⟪a • x, y⟫_𝕜 = a • ⟪x, y⟫_𝕜 := by
...
-- For the specific case of WeightedHilbertSpace (which is an inner product space over ℂ)

lemma inner_smul_left_weighted {a : ℂ} {x y : WeightedHilbertSpace} :

    ⟪a • x, y⟫_ℂ = a • ⟪x, y⟫_ℂ := by

  -- Since WeightedHilbertSpace has an InnerProductSpace ℂ instance
...
lemma inner_smul_left_direct {𝕜 : Type*} {E : Type*} [IsROrC 𝕜] [NormedAddCommGroup E]
...
lemma inner_smul_left_lp (a : ℂ) (x y : lp (fun (p : {p : ℕ // Nat.Prime p}) => primeWeight p) 2) :
...
lemma inner_smul_left_explicit (a : ℂ) (x y : WeightedHilbertSpace) :
  ...
  -- In ℂ, scalar multiplication is just multiplication
...
lemma inner_smul_left_weighted_direct (a : ℂ) (x y : WeightedHilbertSpace) :
  ...
  -- The inner product is the sum Σ_p conj((a • x)(p)) * y(p) * weight(p)

  -- which equals Σ_p conj(a * x(p)) * y(p) * weight(p)

  -- = Σ_p conj(a) * conj(x(p)) * y(p) * weight(p)

  -- = conj(a) * Σ_p conj(x(p)) * y(p) * weight(p)

  -- But for the standard inner product on ℂ, we have ⟪a • x, y⟫ = a * ⟪x, y⟫

  -- not conj(a) * ⟪x, y⟫, so we need to be careful about conventions



  -- Actually, in Mathlib's convention for complex inner products:

  -- ⟪a • x, y⟫ = a * ⟪x, y⟫ (linear in first argument)
...
lemma inner_smul_left {a : ℂ} {x y : WeightedHilbertSpace} : ⟪a • x, y⟫_ℂ = a • ⟪x, y⟫_ℂ :=

  InnerProductSpace.inner_smul_left a x y



This uses the fact that `WeightedHilbertSpace` has an `InnerProductSpace ℂ` instance, and therefore inherits all the properties of inner product spaces, including linearity in the first argument.
...
lemma inner_smul_right : ⟨x, a • y⟩ = conj a • ⟨x, y⟩
...
lemma inner_smul_right {α : Type*} [NormedAddCommGroup α] [InnerProductSpace ℂ α]

    (x y : α) (a : ℂ) : ⟨x, a • y⟩ = conj a • ⟨x, y⟩ := by

  -- First, we'll use the conjugate symmetry property of inner products

  -- We know that ⟨x, a • y⟩ = conj ⟨a • y, x⟩

  rw [inner_conj_symm]



  -- Now we can use the left-linearity property: ⟨a • y, x⟩ = a * ⟨y, x⟩

  rw [inner_smul_left]



  -- Apply conjugate to both sides: conj (a * ⟨y, x⟩) = conj a * conj ⟨y, x⟩

  rw [map_mul]



  -- Use conjugate symmetry again: conj ⟨y, x⟩ = ⟨x, y⟩
...
lemma inner_smul_right {α : Type*} [NormedAddCommGroup α] [InnerProductSpace ℂ α]
  ...
  -- ⟨a • x, y⟩ = a * ⟨x, y⟩ (linearity in first argument)



  -- Step 2: Apply these properties

  calc ⟨x, a • y⟩ = conj ⟨a • y, x⟩ := by rw [inner_conj_symm]

    _ = conj (a * ⟨y, x⟩) := by rw [inner_smul_left]

    _ = conj a * conj ⟨y, x⟩ := by rw [map_mul]

    _ = conj a * ⟨x, y⟩ := by rw [← inner_conj_symm]
...
lemma inner_smul_right (x y : WeightedHilbertSpace) (a : ℂ) :
...
1. Conjugate symmetry: `⟨x, y⟩ = conj ⟨y, x⟩`

2. Linearity in the first argument: `⟨a • x, y⟩ = a * ⟨x, y⟩`
...
lemma conj_real_eq : ∀ r : ℝ, conj r = r
...
lemma conj_real_eq : ∀ r : ℝ, conj r = r := by
...
The proof is quite direct because Mathlib already has the lemma `conj_ofReal` which states that `conj (r : ℂ) = (r : ℂ)` for any real number `r`. This lemma captures exactly what we need: when a real number is coerced to a complex number (which happens implicitly when we apply `conj` to a real number), its conjugate equals itself.
...
The first proof using `conj_ofReal` is the most concise and idiomatic in Mathlib. The key insight is that when we write `conj r` where `r : ℝ`, Lean automatically coerces `r` to `ℂ` before applying `conj`, and the conjugate of a real number (viewed as a complex number with zero imaginary part) is itself.
...
lemma lp_inner_apply (f g : WeightedHilbertSpace) :
    ...
    ⟪f, g⟫_ℂ = ∑' p : {p : ℕ // Nat.Prime p},

      (starRingEnd ℂ) (f.val p) * g.val p * (primeWeight p : ℂ)⁻¹ := by
...
lemma deltaBasis_inner_eq_zero : p ≠ q → ⟪deltaBasis p, deltaBasis q⟫_ℂ = 0 := by
  ...
  · -- Case: r = p and r = q, which contradicts p ≠ q
